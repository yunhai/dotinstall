...
class Lesson extends Base
{
    ...
    private function retrieveLesson(array $fitler = [])
    {
        ...
        $lesson_info = [];
        if ($lessons) {
            $media = $this->getLessonListMedia($lessons);
            $user_id = Auth::id() ?: 0;
            foreach ($lessons as $index => $item) {
                // dd($item);
                // $item = $item->toArray();
                $lessons[$index] = $this->formatLessonList($item, $media, $user_id);
            }
    ...
    private function getLessonListMedia($lessons)
    {
    ...
    private function formatLessonList(array $lesson, $media, $user_id)
    {
